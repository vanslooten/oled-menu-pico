...
class ButtonController:
	"""Three-button controller for SELECT, DOWN and UP on Pico GPIO pins."""

	def __init__( self, select_pin=13, down_pin=14, up_pin=15, debounce_ms=30 ):
		from machine import Pin
		self.select_pin = Pin( select_pin, Pin.IN, Pin.PULL_UP )
		self.down_pin = Pin( down_pin, Pin.IN, Pin.PULL_UP )
		self.up_pin = Pin( up_pin, Pin.IN, Pin.PULL_UP )
	...
	@property
	def button( self ):
		"""Return True while SELECT is pressed."""
		return self.select_pin.value() == 0

	@property
	def rel_position( self ):
		"""Return a single navigation event for a DOWN or UP press."""
		self._poll()
	...
	def _poll( self ):
		now = time.ticks_ms()
		if time.ticks_diff( now, self._last_poll ) < self.debounce_ms:
			return
		self._last_poll = now
		down = self.down_pin.value() == 0
		up = self.up_pin.value() == 0
		if down and not self._last_down:
			self._position = 4
		elif up and not self._last_up:
			self._position = -4
...
class MenuItem:
	__slots__ = ( "owner","code","label", "enabled", "_focus", "cargo", "visible", "_selected", "_selected_ticks" )
...
class RangeControler:
	__slots__ = ( "owner", "parent", "min_val","max_val", "step", "default_val", "value", "selected" )
...
class OLED_MENU:
	def __init__( self, i2c, oled_addr=None, encoder=None, button_pins=(13, 14, 15) ):
		self.i2c = i2c
		...
		# -- Démarrer l'encoder --
		if encoder is None:
			self.enc = ButtonController( *button_pins )
		else:
			self.enc = encoder
